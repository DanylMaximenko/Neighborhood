...
internal sealed class GameForm : Form
{
    ...
    private string _statusText = "LMB: move / interact, RMB: force alert test";
    private int _statusUntilMs;

    // Smooth camera: lerps toward Woody's position each frame
    private float _camX;
    private float _camY;
    private bool  _camInitialized;
    private const float CamLerpSpeed = 8.0f; // world units per ms factor
    ...
    private void OnTick(object? sender, EventArgs e)
    {
        ...
        // Smooth camera follow Woody
        var worldForCam = _logic.World;
        if (worldForCam != null)
        {
            var wo = worldForCam.GetActor("woody", includeOlga: true);
            if (wo != null)
            {
                if (!_camInitialized)
                {
                ...
                {
                    float t = Math.Min(1.0f, CamLerpSpeed * elapsed / 1000.0f);
                    _camX += (wo.Position.X - _camX) * t;
                    _camY += (wo.Position.Y - _camY) * t;
                }
            }
        }
    ...
    private void OnPaint(object? sender, PaintEventArgs e)
    {
        ...
        // Use smooth camera position (lerped toward Woody each frame)
        if (!_camInitialized) { _camX = woody.Position.X; _camY = woody.Position.Y; _camInitialized = true; }
        var camera = new NfhPoint((int)_camX, (int)_camY);
    ...
    private void DrawHouseBackground(Graphics g, WorldState world)
    {
        // house has no position in level.xml (0/0) and no room.
        // It should fill the screen centered, drawn before everything else.
        ...
        // Center the background bitmap on the client area, scaled to render mode
        var drawW = (int)(bmp.Width  * Scale);
        var drawH = (int)(bmp.Height * Scale);
        var x = (ClientSize.Width  - drawW) / 2;
        var y = (ClientSize.Height - drawH) / 2;
        if (Scale == 1.0f)
    ...
    private void DrawSceneSprites(Graphics g, Room? room, NfhPoint camera)
    {
        ...
        // Draw background (house) first, centered on screen, ignoring camera offset.
        // "house" is a top-level layer-0 object -- the full level background image.
        DrawHouseBackground(g, world);
    ...
    private void DrawHud(Graphics g, WorldState world, Room? room, ActorInstance woody)
    {
        ...
        g.DrawString($"Room: {woody.CurrentRoom}", Font, fgBrush, 16, 16);
        g.DrawString($"Woody: {woody.Position.X}/{woody.Position.Y}", Font, fgBrush, 16, 36);
        g.DrawString($"Objects hitboxes: {_objectHitAreas.Count}", Font, fgBrush, 16, 56);

        if (_statusUntilMs > 0)
            g.DrawString(_statusText, Font, accent, 560, 16);

        if (room != null)
            g.DrawString($"Path: {room.Path1.X}/{room.Path1.Y} -> {room.Path2.X}/{room.Path2.Y}", Font, fgBrush, 560, 40);
    }
