...
public class GameLogic
{
    ...
    {
        // Tutorial: if level has HAL actor, show the first target sign
        // (kit/sign is the first waypoint in tutorial_1 - Woody starts in kit)
        if (World != null && layout.Rooms.Any(r =>
            r.Actors.Any(a => a.Name.Equals("hal", StringComparison.OrdinalIgnoreCase))))
        {
            // Show only the first hidden sign as the starting target
            var signNames = new[] { "kit/sign", "lir/sign", "anc/sign" };
            foreach (var signName in signNames)
            {
                if (World.Objects.TryGetValue(signName, out var obj) && !obj.Visible)
                {
                    World.SetObjectVisible(signName, true);
    ...
    /// <summary>
    /// Called when HAL's "target" behavior fires (tutorial levels).
    /// Shows the next waypoint sign in sequence.
    /// 
    /// tutorial_1 sign sequence (derived from trigger.xml order):
    ///   Start: kit/sign visible
    ///   Woody reaches kit/sign  -> show lir/sign
    ///   Woody enters lir/kit    -> (already showing lir/sign)
    ///   Woody reaches lir/sign  -> show anc/sign  
    ///   Woody enters anc/lir    -> (already showing anc/sign)
    ///   Woody reaches anc/sign  -> hide all (tutorial segment done)
    /// </summary>
    protected virtual void OnHalTargetReached()
    {
        if (World == null) return;

        // Find all sign objects ordered by tutorial progression
        var signNames = new[] { "kit/sign", "lir/sign", "anc/sign" };

        // Find the currently visible sign
        int currentIdx = -1;
        for (int i = 0; i < signNames.Length; i++)
        {
            if (World.Objects.TryGetValue(signNames[i], out var obj) && obj.Visible)
            {
                currentIdx = i;
                break;
            }
        ...
        // Hide current sign
        if (currentIdx >= 0)
            World.SetObjectVisible(signNames[currentIdx], false);

        // Show next sign
        int nextIdx = currentIdx + 1;
        if (nextIdx < signNames.Length)
            World.SetObjectVisible(signNames[nextIdx], true);
    }
    ...
    private void WireEvents()
    {
        ...
        Triggers!.BehaviorTriggered += (actor, behavior) =>
        {
            ...
            // HAL = tutorial scripted actor. "target" behavior:
            // The trigger that just fired has an object the player reached.
            // Reveal the NEXT sign in the sequence by making it visible.
            if (actor.Equals("hal", StringComparison.OrdinalIgnoreCase) &&
                behavior.Equals("target", StringComparison.OrdinalIgnoreCase))
            {
                OnHalTargetReached();
            }
