...
public class TriggerSystem
{
    ...
    private bool EvaluateCondition(ActorInstance actor, string behaviorName, TriggerCondition condition)
    {
        ...
        // Build a unique key: actorName:behaviorName:objectName to avoid
        // collisions when multiple behaviors share the same name (e.g. HAL "target")
        var onceKey = behaviorName +
            (condition.ObjectName != null ? ":" + condition.ObjectName : "") +
            (condition.Actor      != null ? "@" + condition.Actor      : "");

        if (condition.Type.Equals("once", StringComparison.OrdinalIgnoreCase) &&
            _firedOnce.Contains(onceKey))
            return false;

        // Optional actor filter: evaluate position condition using the SPECIFIED actor
        // e.g. HAL behavior with actor="woody" -> check woody's position, not HAL's
        ActorInstance evaluationActor = actor;
        if (condition.Actor != null)
        {
            var filtered = _world.GetActor(condition.Actor, includeOlga: true);
            if (filtered == null) return false; // specified actor not in world
            evaluationActor = filtered;
        }
        ...
        {
            conditionMet = EvaluateObjectCondition(evaluationActor, condition);
        }
        else
        {
            conditionMet = EvaluateNoiseCondition(evaluationActor, condition);
        }
