...
public class AnimationSystem
{
    ...
    private void CreateActorAnimators()
    {
        ...
        // Build set of actors actually present in this level (from level.xml)
        var levelActorNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        if (world.Woody    != null) levelActorNames.Add("woody");
        if (world.Neighbor != null) levelActorNames.Add("neighbor");
        if (world.Mother   != null) levelActorNames.Add("mother");
        if (world.Olga     != null) levelActorNames.Add("olga");
        foreach (var a in world.Actors.Keys) levelActorNames.Add(a);

        foreach (var actorName in levelActorNames)
        {
    ...
    private void WireEvents()
    {
        ...
        if (_logic.Brain != null)
        {
            ...
            if (_actorAnimators.TryGetValue("neighbor", out var neighborAnimator))
            {
                neighborAnimator.ActionCompleted += _ => _logic.FinishNeighborReaction();
            }
        }
        ...
        // ActorController movement events -> animator state changes
        if (_logic.Actors != null)
        {
            // Actor started walking between rooms -> switch to walk animation
            _logic.Actors.ActorStartedMoving += (actorName, walkAnim) =>
            {
                if (_actorAnimators.TryGetValue(actorName, out var a))
                    a.SetState(walkAnim);
            };

            // Actor arrived in room -> switch back to idle animation
